...
class vigaContinua:
    ...
    def RNE_qe(self, datos_de_carga, indice_tramo):
        ...
        #Carga linealmente variable
            ...
            FMb = w1*(L-a)**3/(60*L**2)*(3*(L+4*a) - b*(2*L+3*a)/(L-a)*(1 +\
                ...
                b**2/(L-a)**2) - 3*b**3/(L-a)**2*(1 + (5*L-4*b)/(L-a)))
    ...
    #Fuerza cortante en una sección (sin considerar reacciones)
    def V_sqe(self, xt, datos_de_carga, indice_tramo):
        ...
        if tipo_carga == 1:  # datos_de_carga = (1, P, a)
            ...
            if a < xt <= L:  # A la derecha de P
                return -P
            else:  # En cualquier otro lugar
                return 0
        ...
        # Carga lineal
            ...
            c = L - a - b # Longitud de distribución de la carga [m]
            x = xt # Cambio de variabla para acortar
            if a < x < L - b:  # Entre los límites de la carga
                R1 = (qf - q0)/(2*c)*(x - a)**2 # Resultante triángulo
                R2 = q0*(x - a) # Resultante rectángulo
                return -R1 - R2
            elif L - b <= x <= L:  # A la derecha de la carga
                R1 = (qf - q0)*c/2 # Triángulo total
                R2 = q0*c # Rectángulo total
                return -R1 - R2
            else:  # En cualquier otro lugar
                return 0
    ...
    #Momento flector en una sección (sin considerar reacciones)
    def M_sqe(self, xt, datos_de_carga, indice_tramo):
        ...
        #Momento concentrado
            ...
            if a < xt <= L: #A la derecha del momento
                return -M
            else: #En cualquier otro lugar
                return 0
        ...
        #Carga linealmente variable
            ...
            Rt = (qf - q0)*c/2 # Resultante de la parte triangular
            Rr = q0*c # Resultante de la parte rectangular

            if a < x <= L - b: #Entre los límites de la carga
                rt = (qf - q0)/(2*c)*(x - a)**2 # Parte triangular
                rr = q0*(x - a) # Parte rectangular
                M2 = -rt*(x - a)/3 - rr*(x - a)/2 # Momento flector
                return M2
            elif L - b < x <= L: #A la derecha de la carga
                M3 = -Rt*(x - a - 2/3*c) - Rr*(x - a - c/2) # Momento flector
                return M3
            else: # En cualquier otro lugar
                return 0
    ...
    #Desplaz. vertical de un punto del eje (viga biempotrada)
    def D_sqe(self, xt, datos_de_carga, indice_tramo):
        ...
        #Carga linealmente variable
            ...
            vt1 = c*x**2*(-2*L**4 + L**3*a - 4*L**3*b + 3*L**3*x +\
                4*L**2*a**2 - 2*L**2*a*b - 4*L**2*a*x - 6*L**2*b**2 +\
                6*L**2*b*x - 3*L*a**3 + 6*L*a**2*b - L*a**2*x -\
                9*L*a*b**2 - 2*L*a*b*x + 12*L*b**3 + 9*L*b**2*x +\
            ...
            vt3 = (-L + x)**2*c*(4*L**4 + 3*L**3*a - 12*L**3*b -\
            ...
            # A la derecha de la carga
            vr3 = (-L + x)**2*c*(L**4 + L**3*a - 3*L**3*b + L**2*a**2 -\
                2*L**2*a*b + 3*L**2*b**2 + L*a**3 - L*a**2*b + L*a*b**2 -\
                L*b**3 + x*(-2*L**3 - 2*L**2*a + 2*L**2*b - 2*L*a**2 + 2*L*b**2 +\
                2*a**3 - 2*a**2*b + 2*a*b**2 - 2*b**3))/(24*L**3)

            if 0 < xt <= a: # A la izquierda de la carga
                d1 = (vr1*q0 + vt1*(qf - q0))/EI
                return d1
            elif a < xt <= a + c: # Entre los límites de la carga
                d2 = (vr2*q0 + vt2*(qf - q0))/EI
                return d2
            elif a + c < xt <= L: # A la derecha de la carga
    ...
    def diagramas(self):
        ...
        for it in range(NT): # Recorre los tramos
            ...
            M_emp = V_emp*Xt - M1 # M de empotramiento
